...
def solution(lines: list[str]):
    ...
    for raw_map in stuff[1:]:
        ...
        for raw_row in raw_rows:
            ...
            src = nums[1]
            rng = nums[2]
            discontinuities.extend([src, src + rng])
            range_offset_map[(src, src + rng)] = dst - src
        maps.append(range_offset_map)

    seed_ranges = [int(s) for s in stuff[0].partition(':')[2].strip().split()]
    seeds = []
    for i in range(0, len(seed_ranges) - 2, 2):
        init = seed_ranges[i]
        rng = seed_ranges[i + 1]
        for j in range(rng):
            if init + j in discontinuities:
                seeds.append(init + j)
    seeds.append((seed_ranges[-2] + seed_ranges[-1] - 1))
